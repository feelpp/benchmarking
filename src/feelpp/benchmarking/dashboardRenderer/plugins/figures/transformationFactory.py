            ...
            for aggregation in self.aggregations:
                if aggregation.column in df.columns:
                    ...
                    if aggregation.agg.startswith("filter:"):
                        ...
                        pivot = pivot.groupby(agg_columns)[self.dimensions["yaxis"]].agg(aggregation.agg).reset_index()

        return pivot.pivot(index=index,values=self.dimensions["yaxis"],columns=self.dimensions["color_axis"])
...
class TransformationStrategyFactory:
    ...
    def create(plot_config):
        ...
        dimensions = {
            ...
            "color_axis" : plot_config.color_axis.parameter,
            "extra_axes" : [extra.parameter for extra in plot_config.extra_axes],
            "yaxis" : plot_config.yaxis.parameter
        }
