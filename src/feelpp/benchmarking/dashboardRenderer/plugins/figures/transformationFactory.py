...
class TransformationStrategy:
    ...
    def __init__(self,dimensions,aggregations):
    ...
    def updateDimensions(self,df):
        ...
        for k,v in self.dimensions.items():
            if isinstance(v,str):
                dimensions[k].parameter = self.chooseColumn(v.parameter,df)
            elif isinstance(v,list):
                dimensions[k] = []
                for value in v:
                    dimensions[k][value].parameter = self.chooseColumn(value.parameter,df)
            else:
                dimensions[k] = v
        self.dimensions = dimensions
    ...
    def filterAndRename(self,df,axis):
        ...
        new_df = df.loc[df[col].isin(list(filters.keys())),:]
        new_df.loc[:,col] = new_df[col].apply(lambda x : filters.get(x,x) )
        return new_df

class PerformanceStrategy(TransformationStrategy):
    """ Strategy that pivots a dataframe on given dimensions"""
    def __init__(self,dimensions,aggregations):
        super().__init__(dimensions,aggregations)
    ...
    def calculate(self,df):
        ...
        for axis_name, axis in self.dimensions.items():
            if axis_name == "extra_axes":
                ...
                df = self.filterAndRename(df,axis)

        index = []
        if self.dimensions["secondary_axis"] and self.dimensions["secondary_axis"].parameter in df.columns:
            index.append(self.dimensions["secondary_axis"].parameter)
        if self.dimensions["xaxis"] and self.dimensions["xaxis"].parameter in df.columns:
            index.append(self.dimensions["xaxis"].parameter)
        for axis in self.dimensions["extra_axes"]:
            if axis.parameter in df.columns:
                index.append(axis.parameter)

        pivot = df

        if self.aggregations:
            self.updateAggregations(df)
            agg_columns = index + [self.dimensions["color_axis"].parameter] + [a.column for a in self.aggregations if a.column in df.columns]
            ...
            for aggregation in self.aggregations:
                if aggregation.column in df.columns:
                    ...
                    if aggregation.agg.startswith("filter:"):
                        ...
                        pivot = pivot.groupby(agg_columns)[self.dimensions["yaxis"].parameter].agg(aggregation.agg).reset_index()

        return pivot.pivot(index=index,values=self.dimensions["yaxis"].parameter,columns=self.dimensions["color_axis"].parameter)
...
class RelativePerformanceStrategy(PerformanceStrategy):
    ...
    def calculate(self,df):
        """ Pivots dataframe, setting self.dimensions["color_axis"] values as columns, "value" as cell values, and dimensions as indexes.
...
class TransformationStrategyFactory:
    ...
    def create(plot_config):
        ...
        """
        dimensions = {
            "xaxis" : plot_config.xaxis,
            "secondary_axis" : plot_config.secondary_axis,
            "color_axis" : plot_config.color_axis,
            "extra_axes" : plot_config.extra_axes,
            "yaxis" : plot_config.yaxis
        }
        aggregations = plot_config.aggregations

        if plot_config.transformation == "performance":
            return PerformanceStrategy(dimensions,aggregations)
        elif plot_config.transformation == "speedup":
            return SpeedupStrategy(dimensions,aggregations)
        elif plot_config.transformation == "relative_performance":
            return RelativePerformanceStrategy(dimensions,aggregations)
        else:
