...
class TikzFigure(Figure):
    ...
    def __init__(self,plot_config, renderer_filename):
        super().__init__(plot_config)
        self.template_dirpath = f"{Path(__file__).resolve().parent}/templates/"
    ...
    def createMultiindexFigure(self, df, data_dirpath, **args):
    ...
    def createSimpleFigure(self, df,data_dirpath, **args):
...
class TikzScatterFigure(TikzFigure):
    ...
    def __init__(self, plot_config, fill_lines = []):
        super().__init__(plot_config, "scatterChart.tex.j2")
        self.fill_lines = fill_lines

    def createFigure(self, df, data_dir):
        return super().createFigure(df, data_dir, fill_lines = self.fill_lines)

class TikzTableFigure(TikzFigure):
    """ Concrete Figure class for pgfplots table"""
    def __init__(self, plot_config):
        super().__init__(plot_config, "tableChart.tex.j2")

class TikzStackedBarFigure(TikzFigure):
    """ Concrete Figure class for pgfplots stacked bar figure"""
    def __init__(self, plot_config):
        super().__init__(plot_config, "stackedBarChart.tex.j2")

    def createFigure(self,df, data_dir):
        return super().createFigure(df,data_dir, colors=self.xcolors[:len(df.columns)])

class TikzGroupedBarFigure(TikzFigure):
    """ Concrete Figure class for pgfplots grouped bar figure"""
    def __init__(self, plot_config):
        super().__init__(plot_config, "groupedBarChart.tex.j2")
