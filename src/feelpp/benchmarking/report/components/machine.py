...
class Machine(BaseComponent):
    ...
    def initModules(self, base_dir, renderer, parent_id = "supercomputers"):
        ...
        for application, use_cases in self.tree.items():
            ...
            for use_case in use_cases:
                use_case.initModules(os.path.join(base_dir,self.id,application.id), renderer, parent_id = f"{self.id}-{application.id}", self_tag_id = f"{self.id}-{application.id}-{use_case.id}")