...
The command successfully generates the report, creating the directory `output` and saving your rendered document as `output/config.adoc` and have the following content:
...
<script>
  document.addEventListener('DOMContentLoaded', () => {
    ...
    initTable(table, 'filter_Performance Summary_table');
  });
</script>
++++
